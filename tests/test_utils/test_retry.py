...
from src.utils.retry import retry_on_rate_limit, retry_with_backoff
...
class TestRetryDecorator:
    ...
    def test_retry_on_exception(self):
        ...
        mock_func = Mock(
            side_effect=[ValueError("First failure"), ValueError("Second failure"), "success"]
        )

        @retry_with_backoff(
            max_retries=3, initial_delay=0.1, exceptions=(ValueError,)  # Fast retry for testing
        )
        def test_func():
            return mock_func()
    ...
    def test_exponential_backoff(self):
        ...
        @retry_with_backoff(
            max_retries=3, backoff_factor=2.0, initial_delay=0.1, exceptions=(ValueError,)
        )
        def test_func():
    ...
    def test_specific_exception_only(self):
        ...
        @retry_with_backoff(
            max_retries=3, initial_delay=0.1, exceptions=(ValueError,)  # Only retry ValueError
        )
        def test_func():
            return mock_func()
    ...
    def test_retry_callback(self):
        ...
        @retry_with_backoff(
            max_retries=2, initial_delay=0.1, exceptions=(ValueError,), on_retry=on_retry
        )
        def test_func():
...
class TestRateLimitRetry:
    ...
    def test_rate_limit_retry(self):
        ...
        mock_func = Mock(side_effect=[Exception("Rate limit"), "success"])
